...
class ImagesFromFolder_Original(data.Dataset):
    def __init__(self, args, is_cropped, root = '/path/to/frames/only/folder', iext = 'png', replicates = 1,debug=False):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
    ...
    def __getitem__(self, index):
        ...
        # TODO read flow from folder KITTY
        abg = torch.ones(images.size()[0:1] + (2,) + images.size()[-2:])
    ...
    def __len__(self):
        return self.size * self.replicates
#######################################################################################################
class ImagesFromFolder_working(data.Dataset):
    def __init__(self, args, is_cropped, root = './KITTI/images_train', iext = 'png', replicates = 1, debug=False):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
        ...
        if (mode=='images_train' ):
            flows = sorted( glob( join('./KITTI/flow_noc_train', '*.' + iext) ) )
        elif (mode=='images_test' ):
            flows = sorted( glob( join('./KITTI/flow_noc_test', '*.' + iext) ) )
        else:
            flows = []
           
        if debug:
            print(flows)
            print('len of flows == '+str(len(flows)))
            print('************************************')
            print(cdm)
            print('len of cdm == '+str(len(cdm)))
            print('************************************')
    ...
    def __getitem__(self, index,debug=True):
        index = index % self.size

        img1 = frame_utils.read_gen(self.image_list[index][0])
        img2 = frame_utils.read_gen(self.image_list[index][1])

        flow12 = frame_utils.read_gen_flow(self.flow_list[index]) # open-cv dependent !!!!!!!!!!!!
        ...
        return [images], [flow12]

    def __len__(self):
        return self.size * self.replicates

class ImagesFromFolder(data.Dataset):
    def __init__(self, args, is_cropped, root = './KITTI/images_train', iext = 'png', replicates = 1, debug=True):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
        ...
        if (mode=='images_train' ):
            flows = sorted( glob( join('./KITTI/flow_noc_train', '*.' + iext) ) )
            cdm =   sorted( glob( join('./KITTI/mask_train/CDM', '*.' + iext) ) )
        elif (mode=='images_test' ):
            flows = sorted( glob( join('./KITTI/flow_noc_test', '*.' + iext) ) )
            cdm =   sorted( glob( join('./KITTI/mask_test/CDM', '*.' + iext) ) )
    ...
    def __getitem__(self, index,debug=False):
        ...
        img1 = cv2.resize(img1, dsize = (1220,370),  interpolation=cv2.INTER_LINEAR)
        img2 = cv2.resize(img2, dsize = (1220,370),  interpolation=cv2.INTER_LINEAR)
        flow12 = cv2.resize(flow12, dsize = (1220,370),  interpolation=cv2.INTER_LINEAR)
        if debug:
