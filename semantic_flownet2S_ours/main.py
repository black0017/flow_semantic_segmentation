...
if __name__ == '__main__':
    ...
    tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset', default='ImagesFromFolder', 
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': './KITTI/images_train'})
    
    tools.add_arguments_for_module(parser, datasets, argument_for_class='validation_dataset', default='ImagesFromFolder', 
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': './KITTI/images_test',
                                                        'replicates': 1})
    print('TODO: check this when you need to do inference')
    tools.add_arguments_for_module(parser, datasets, argument_for_class='inference_dataset', default='ImagesFromFolder_Original', 
                                    skip_params=['is_cropped'],
                                    parameter_defaults={'root': './KITTI/images_test',
                                                        'replicates': 1})
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target, cdm ) in enumerate(progress):
            data, target , cdm  = [Variable(d, volatile = is_validate) for d in data], [Variable(t, volatile = is_validate) for t in target], [Variable(q, volatile = is_validate) for q in cdm]

            if args.cuda and args.number_gpus == 1:
                data, target , cdm = [d.cuda(async=True) for d in data], [t.cuda(async=True) for t in target],[q.cuda(async=True) for q in cdm]
